...
| GUI app lifecycle | `NSApplication.sharedApplication`, `run`, `terminate:` | Win32 message loop with `PeekMessageW`, `TranslateMessage`, `DispatchMessageW`, `WM_QUIT` | Partial | `lib/echoes/gui_win32.rb` | Basic loop exists. Native close requests and menu exit now share a shutdown path, but app-level services are still not equivalent to AppKit. |
