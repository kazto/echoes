  ...
  class GUI
    class Win32Window
      def initialize(gui, command:, rows:, cols:, font_size:)
        ...
        # 既存のタブ・ペインのハンドラを Windows 用にバインドし直す
      ...
      def wire_screen_handlers(pane)
        ...
        screen.notification_handler = ->(title, message) { post_notification(title, message) }
      ...
      def draw_multicell_text(hdc, cell, x, y, fg_color, bg_color)
        mc = cell.multicell
        return if mc[:sixel]
      ...
      def fill_rect_color(hdc, left, top, right, bottom, color)
      ...
      def draw_text_decorations(hdc, x, y, width, color, underline:, strikethrough:, height: @cell_height)
      ...
      def blit_kitty_placement(hdc, pl, px, py, pane_rows)
